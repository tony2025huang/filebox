...
| Per-user read-only window (v012 功能6 first part) | designed | Admin sets a one-time from/until window per user; inside it the user can only view/download (all write operations blocked with 403 READ_ONLY); admin sets it in the user-edit modal; frontend disables write entries with a notice bar. Design confirmed; codex batch D in progress. |
