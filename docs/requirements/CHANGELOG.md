...
- 缺陷1：日志时间筛选弹出面板增加「确定」「清空」按钮（common.confirm/clear 三语 + time-range-actions 样式）；未确定不生效
- 缺陷2+3：日志页（67 条）与我的文件页分页不显示修复——根因：模板中误用 `pageSize.value`（Vue 模板 ref 自动解包，`.value` 为未定义 → 分页条件恒 false）
- 缺陷4：目录导航规范化（反斜杠转正斜杠 + 去首尾斜杠）——修复点击存在文件的目录进入后提示「目录无效」
...
- 缺陷7：我的文件目录批量操作——目录行增加复选框（selectedFolderIds），支持批量删除（逐目录删除+确认+三语提示）
