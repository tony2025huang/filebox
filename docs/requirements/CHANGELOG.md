...
- 【业务确认】触发语义：本地 FileBox 目录发生变更后，协调器以 30 秒去抖（最后一次变更后重启倒计时）执行一次同步；若同步进行中再次发生变更，合并为「完成后的一次补跑」，同一任务绝不并发执行；手动/调度/触发共用同一任务锁与 `executeSyncTask` 执行路径（所有者禁用/只读/目标缺失行为一致）。
- 【业务确认】变更来源覆盖：普通上传完成（含覆盖/重命名替换）、文件夹新建/重命名（旧+新路径）/删除、文件删除、批量删除（含 force 递归目录树，通知被删目录树与受影响文件目录）、收集上传完成（若为属主创建文件）；分片上传、失败/不完整的分块与秒传命中不触发（秒传未在目标目录产生新文件）。
- 【业务确认】路径匹配规则：根源空路径匹配该用户全部变更；非根源任务仅匹配「变更目录 == 源目录」或「变更目录位于源目录子树」。
- 【业务确认】生命周期与重启语义：协调器随进程启动加载 registry，任务新建/编辑/删除/启停后即时刷新；进程退出停止全部去抖定时器；服务重启不补跑重启期间错过的变更（明确不追溯执行，文档说明）。
- 【开发拟定】前端 SyncView：调度下拉新增「变更触发」，非 push 时禁用并在保存/切换方向时给出提示；触发模式下隐藏 cron、展示 30 秒去抖说明；任务行显示「变更触发」状态。新增协调器/校验/端点与文件夹创建端点联动测试（`sync_trigger_test.go`）。
