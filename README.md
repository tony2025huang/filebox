...
- v020 用户反馈优化批次（11 项）：删除/撤销等 12 处原生 confirm 统一为 FileBox 自定义确认弹窗（队列式，web/src 零 window.confirm）；批量 ZIP 下载失败原因分类透传（磁盘满/无写入权限/IO 错误）且文件名带时间戳 `filebox-batch-YYYYMMDD-HHMMSS.zip`；传输抽屉支持批量 暂停/继续/终止（上传终止释放服务端配额，下载支持 Range 断点续传）；顶栏「传输」按钮图标+文字；我的收集双刷新按钮去重并修复其分页条不显示隐患；分享管理副标题误显「复制链接」修复（shares.intro 键拆分）；聚合分享编辑弹窗对齐单分享（信息网格+链接+小时制延期/增次+成员管理）；同步任务日志文件详情改为弹窗展示（逐文件状态徽标）；同步详情弹窗加宽至 1280px；日志失败原因三语本地化补齐（logReason 38 键三语一致）；传输图标改为上下箭头。
- v019 用户反馈修复批次：日志时间筛选面板增加「确定/清空」按钮（草稿态，未点确定关闭不生效）；日志/我的文件/我的分享/聚合分享四页分页显示修复（模板 ref 解包 bug）；目录点击「目录无效」修复（前端导航规范化 + 后端目录列表过滤归一化：剥离历史 `files/` 前缀、剔除 v010 反斜杠遗留与 `.`/`..` 目录）；聚合分享卡片精简为 眼睛/复制/删除 三图标（编辑合入小眼睛弹窗）；同步日志弹窗加宽至 1100px；目录支持批量勾选/批量删除/批量重命名。
- v018 功能增强：修复顶栏「系统设置」中文显示（section→键映射，admin→nav.system）；聚合分享支持成员文件增删与属性编辑（眼睛图标查看文件范围 + 编辑弹窗：改有效期/上限）；我的收集「复制链接」与「查看已收文件」按钮同行；同步任务执行中实时展示传输进度（当前文件/文件数/字节/速率，`GET /api/sync/tasks/{id}/progress` 轮询）；同步日志改为表格列（开始/结束/状态/下次执行/文件数/大小/详情展开）；日志时间筛选合并为「时间范围」下拉面板；我的文件/我的分享/我的收集/日志四页支持每页条数选择（10/20/50/100）与页码跳转；我的文件目录与文件合并为同一列表（目录在前、点击进入）。
