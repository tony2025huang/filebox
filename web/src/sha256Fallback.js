// sha256Fallback 是零依赖的流式 SHA-256 实现，主线程回退路径与 Web Worker 共用同一份代码，
// 避免两份实现漂移（v031-A）。Worker 内优先使用 WASM（hash-wasm），本模块作为其兜底。
// sha256Fallback is the dependency-free streaming SHA-256 shared by the main-thread fallback and the
// Web Worker so the two cannot drift apart (v031-A). The worker prefers WASM (hash-wasm) and uses
...
// IncrementalSHA256 hashes chunks without allocating a full-file buffer.
// IncrementalSHA256 在不创建完整文件缓冲区的前提下计算分块哈希。
...
// createSha256 返回与 hash-wasm createSHA256 相同形状的流式 hasher（update/digest），
// 让 Worker 的两条实现可以互换使用。
// createSha256 returns a streaming hasher shaped like hash-wasm's createSHA256 (update/digest) so the
// worker can swap implementations without branching at every call site.
export function createSha256() {
  const hasher = new IncrementalSHA256()
  return {
    engine: 'js',
    update(chunk) { hasher.update(chunk) },
    digest() { return hasher.hexDigest() }
  }
}
