    ...
    <section class="login-panel">
      <div class="login-form-wrap">
        ...
        <form @submit.prevent="submit">
          ...
          <button class="primary-button submit-button" :disabled="loading"><span>{{ loading ? t('login.submitting') : totpMode ? t('login.totpSubmit') : t('login.submit') }}</span><ArrowRight :size="18" /></button>
...
async function submit() {
  ...
  error.value = ''
  try {
    const body = await api(totpMode.value ? '/api/auth/totp' : '/api/auth/login', { method: 'POST', body: JSON.stringify(totpMode.value ? { totpChallenge: totpChallenge.value, code: totpCode.value } : form) })
