...
async function startQueue() { if (running.value || !queue.value.length) return; running.value = true; error.value = ''; notice.value = ''; for (const item of queue.value) { if (item.progress === 100) continue; item.failed = false; try { const result = await uploadOne(item); item.progress = 100; item.status = t('collection.completed'); completed.value.push(result || { name: item.file.name, size: item.file.size }) } catch (err) { item.failed = true; item.status = err.message; error.value = `${item.file.name}: ${err.message}` } } running.value = false; if (!error.value) notice.value = t('collection.allCompleted'); await loadMeta() }
