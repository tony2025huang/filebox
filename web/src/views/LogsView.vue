<template>
  <main class="app-shell">
    ...
    <section class="content-wrap">
      ...
      <div v-if="user.role === 'admin'" class="settings-panel"><div><p class="eyebrow">{{ t('logs.retentionEyebrow') }}</p><strong>{{ t('logs.retention') }}</strong><span class="muted">{{ t('logs.retentionCopy') }}</span></div><div class="settings-control"><input v-model.number="settings.logRetentionDays" type="number" min="0" :aria-label="t('logs.retentionDays')" /><span>{{ t('common.days') }}</span><button class="secondary-button" :disabled="saving" @click="saveSettings"><Save :size="16" /> {{ t('common.save') }}</button></div></div>
...
import { ChevronLeft, ChevronRight, FolderOpen, LoaderCircle, LogOut, RefreshCw, Save, ScrollText, Search, KeyRound, Shield } from 'lucide-vue-next'

const router = useRouter(); const user = ref(JSON.parse(localStorage.getItem('filebox_user') || '{}')); const logs = ref([]); const actions = ref([]); const users = ref([]); const total = ref(0); const page = ref(1); const pageSize = 20; const loading = ref(false); const saving = ref(false); const error = ref(''); const notice = ref(''); const keywordInput = ref(''); const filters = reactive({ action: '', result: '', keyword: '', userId: '' }); const settings = reactive({ logRetentionDays: 30, lockThreshold: 5, autoUnlockEnabled: true, autoUnlockMinutes: 5, defaultLang: 'zh-CN', themeColor: '#1b998b', passwordMinLength: 8, passwordComplexity: 3, ipLockWindowMinutes: 10, ipLockThreshold: 50, ipAutoUnlockEnabled: true, ipUnlockMinutes: 30 })
...
async function loadAdminData() { if (user.value.role !== 'admin') return; try { const [settingsBody, usersBody] = await Promise.all([api('/api/admin/settings'), api('/api/admin/users?page=1&pageSize=100')]); Object.assign(settings, settingsBody.data); users.value = usersBody.data.items } catch (err) { error.value = err.message } }
function applyFilters() { page.value = 1; filters.keyword = keywordInput.value.trim(); loadLogs() }
async function saveSettings() { saving.value = true; error.value = ''; try { const body = await api('/api/admin/settings', { method: 'PUT', body: JSON.stringify(settings) }); Object.assign(settings, body.data); notice.value = t('notice.settingsSaved') } catch (err) { error.value = err.message } finally { saving.value = false } }
