...
const user = ref(JSON.parse(localStorage.getItem('filebox_user') || '{}')); const logs = ref([]); const actions = ref([]); const actionsLoading = ref(true); const users = ref([]); const total = ref(0); const page = ref(1); const pageSize = 20; const loading = ref(false); const error = ref(''); const notice = ref(''); const keywordInput = ref(''); const timeRangeOpen = ref(false); const filters = reactive({ action: '', result: '', keyword: '', userId: '', from: '', to: '' })
