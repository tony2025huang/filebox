...
async function batchDeleteFolders() {
  ...
  if (!window.confirm(t('confirm.deleteFolders', { count: selectedFolderIds.size }))) return
  batchDeletingFolders.value = true
  try {
    for (const id of [...selectedFolderIds]) {
      try { await api(`/api/folders/${id}`, { method: 'DELETE' }) } catch { /* 单目录失败继续其余 */ }
      selectedFolderIds.delete(id)
    }
    notice.value = t('notice.foldersDeleted')
