<template>
  <main class="app-shell" :class="{ 'read-only': readOnly }">
    ...
    <section class="content-wrap" @dragover.prevent="!readOnly && (dragging = true)" @dragleave.prevent="dragging = false" @drop.prevent="!readOnly && handleDrop($event)">
    ...
    <aside class="transfers-drawer" :class="{ open: transfersOpen }" aria-label="transfers">
      ...
      <div class="transfers-body">
        <div class="transfers-tabs" role="tablist">
          ...
          <button class="transfers-tab" :class="{ active: transfersTab === 'done' }" role="tab" :aria-selected="transfersTab === 'done'" @click="transfersTab = 'done'">{{ t('files.transferDoneTab') }}</button>
        </div>
        <div v-if="transfersTab === 'active'" class="transfer-tab-panel" role="tabpanel">
        ...
        <div v-else class="transfer-tab-panel" role="tabpanel">
          ...
          <div v-for="item in uploadsDone" :key="item.id" class="transfer-row transfer-finished-row"><FileUp :size="16" class="transfer-icon" /><strong class="transfer-finished-name" :title="item.relPath || item.file?.name || item.name">{{ item.relPath || item.file?.name || item.name }}</strong><span class="transfer-finished-size">{{ formatBytes(item.file?.size || item.size || 0) }}</span><span class="finished-result finished-success">{{ t('files.finishedSuccess') }}</span><button class="icon-button" :title="t('files.clearFinished')" @click="clearFinishedUpload(item)"><Trash2 :size="15" /></button></div>
...
const uploadsActive = computed(() => uploads.value.filter(item => !isUploadComplete(item)))
const uploadsDone = computed(() => uploads.value.filter(isUploadComplete))
...
function toggleAllUploads() { if (allUploadsSelected.value) uploadsActive.value.forEach(item => selectedUploadIds.delete(item.id)); else uploadsActive.value.forEach(item => selectedUploadIds.add(item.id)) }
function toggleAllDownloads() { if (allDownloadsSelected.value) downloadsActive.value.forEach(item => selectedDownloadIds.delete(item.id)); else downloadsActive.value.forEach(item => selectedDownloadIds.add(item.id)) }
...
function pauseAllUploads() { pauseUploads(uploadsActive.value.slice()) }
...
function clearFinishedUpload(item) { if (!isUploadComplete(item)) return; dismissUpload(item) }
...
function resumeAllDownloads() { return resumeDownloads(downloadsActive.value.slice()) }
...
function terminateAllDownloads() { terminateDownloads(downloadsActive.value.slice()) }
