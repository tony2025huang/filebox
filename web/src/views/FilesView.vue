<template>
  <main class="app-shell" :class="{ 'read-only': readOnly }">
    ...
    <section class="content-wrap">
      <div class="page-heading"><div><p class="eyebrow">WORKSPACE / {{ user.role === 'admin' ? t('files.workspaceAdmin') : t('files.workspaceUser') }}</p><h1>{{ t('files.heading') }}</h1><p class="muted">{{ t('files.copy') }}</p></div><div class="quota-block"><div class="quota-label"><span>{{ t('files.quota') }}</span><strong>{{ formatBytes(user.usedBytes) }}</strong> <span class="quota-total">/ {{ formatBytes(user.quotaBytes) }}</span></div><div class="progress-track"><span :style="{ width: quotaPercent + '%' }"></span></div></div></div>
    ...
    <aside class="transfers-drawer" :class="{ open: transfersOpen }" aria-label="transfers">
      ...
      <div class="transfers-body">
        ...
        <div v-if="transfersTab !== 'done'" class="transfer-tab-panel" role="tabpanel">
          ...
          <div v-if="tabUploads.length" class="transfer-batch-toolbar"><span class="transfer-selection-count">{{ t('files.selectedTransfers', { count: selectedUploadIds.size }) }}</span><div class="transfer-batch-actions"><div class="transfer-batch-group selected-group"><button class="secondary-button" :disabled="uploadBatchBusy || !selectedUploadIds.size || !canPauseSelectedUploads" @click="pauseSelectedUploads"><Pause :size="14" /> {{ t('files.pauseSelected') }}</button><button class="secondary-button" :disabled="uploadBatchBusy || !selectedUploadIds.size || !canResumeSelectedUploads" @click="resumeSelectedUploads"><Play :size="14" /> {{ t('files.resumeSelected') }}</button><button class="secondary-button danger-action" :disabled="uploadBatchBusy || !selectedUploadIds.size || !canTerminateSelectedUploads" @click="terminateSelectedUploads"><X :size="14" /> {{ t('files.terminateSelected') }}</button></div><div class="transfer-batch-group all-group"><button class="secondary-button" :disabled="uploadBatchBusy || !canPauseAllUploads" @click="pauseAllUploads"><Pause :size="14" /> {{ t('files.pauseAll') }}</button><button class="secondary-button" :disabled="uploadBatchBusy || !canResumeAllUploads" @click="resumeAllUploads"><Play :size="14" /> {{ t('files.resumeAll') }}</button><button class="secondary-button danger-action" :disabled="uploadBatchBusy || !canTerminateAllUploads" @click="terminateAllUploads"><X :size="14" /> {{ t('files.terminateAll') }}</button></div></div></div>
