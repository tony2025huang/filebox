<template>
  <main class="login-shell">
    ...
    <section class="login-intro"><BrandLogo variant="login" /><h1>{{ t(forced ? 'password.heading' : 'password.selfHeading') }}</h1><p class="intro-copy">{{ t(forced ? 'password.copy' : 'password.selfCopy') }}</p><div class="intro-rule"></div><p class="intro-meta">{{ t('login.meta') }}</p></section>
    <section class="login-panel"><div class="login-form-wrap"><div class="mobile-brand"><BrandLogo variant="login" compact /></div><h2>{{ t('password.title') }}</h2><p class="muted">{{ t('password.policy', policy) }}</p><form @submit.prevent="submit"><label>{{ t('password.old') }}<input v-model="form.oldPassword" type="password" autocomplete="current-password" required /></label><label>{{ t('password.new') }}<input v-model="form.newPassword" type="password" autocomplete="new-password" required /></label><label>{{ t('password.confirm') }}<input v-model="confirmation" type="password" autocomplete="new-password" required /></label><p v-if="error" class="alert error">{{ error }}</p><button class="primary-button submit-button" :disabled="loading"><span>{{ loading ? t('password.submitting') : t('password.submit') }}</span><ArrowRight :size="18" /></button></form><BrandFooter /></div></section>
  </main>
</template>
