...
function syncDirection() {
  ...
  if (taskForm.scheduleType === 'triggered' && taskForm.direction !== 'push') {
    taskForm.scheduleType = 'once'
    taskForm.cron = ''
    formError.value = t('sync.triggeredRequiresPush')
  }
}
// scheduleTypeLabel 展示调度方式：周期任务显示 cron，触发任务显示“变更触发”，其余为一次性。
function syncScheduleLabel(item) { if (item.scheduleType === 'periodic') return item.cron || t('sync.periodic'); if (item.scheduleType === 'triggered') return t('sync.triggered'); return t('sync.once') }
function syncScheduleStateLabel(item) { const enabled = item.enabled ? t('sync.enabled') : t('sync.disabled'); return item.scheduleType === 'triggered' ? `${t('sync.triggeredRow')} · ${enabled}` : enabled }
// onScheduleChange 在选择“变更触发”时提示约束；非周期一律清空 cron。
...
function closeDetails() { details.value = null; closeFileDetail() }
function openFileDetail(entry) { fileDetail.value = { entry, raw: entry.detail || '', rows: parseFileDetail(entry.detail) } }
function closeFileDetail() { fileDetail.value = null }
function parseFileDetail(detail = '') {
  ...
  return raw.split(/\r?\n/).filter(line => line.trim()).map(line => {
    ...
    const hasFilePrefix = separatorIndex > 0 && !isOverallDetailPrefix(prefix)
    const file = hasFilePrefix ? prefix.trim() : ''
    const message = hasFilePrefix ? line.slice(separatorIndex + 2).trim() : line
    const statusKey = detailStatusKey(line, !file)
    return { raw: line, file, message, statusKey, statusClass: statusKey }
  })
}
function isOverallDetailPrefix(prefix) {
  return /^(?:同步|读取|创建|传输|检查|任务|目标系统|source|target|sync|transfer|connection|permission|disk).*?(?:失败|错误|不可用|取消|跳过|failed|failure|error|unavailable|cancelled|canceled|timeout|refused|denied)/i.test(prefix)
}
function detailStatusKey(line, isOverall) {
  if (/uploaded|downloaded/i.test(line)) return 'success'
  if (/skipped/i.test(line)) return 'skipped'
  if (/(失败|不可用|取消|拒绝|错误|超时|connection refused|permission denied|no such file|disk check failed|failed|failure|error|unavailable|cancelled|canceled|timeout|timed out|refused|denied|not found|unable|cannot)/i.test(line)) return 'failure'
  return isOverall ? 'overallError' : 'note'
}
