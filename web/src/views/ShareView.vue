...
const downloadUrl = computed(() => `/api/files/shared/${encodeURIComponent(token.value)}/download`); const previewAllowed = computed(() => canPreview(meta.value.mime)); const downloadExhausted = computed(() => meta.value.maxDownloads > 0 && (meta.value.downloadAvailable === false || meta.value.downloadCount >= meta.value.maxDownloads))
...
async function loadMeta() { loading.value = true; error.value = ''; downloadError.value = ''; try { const body = await api(`/api/files/shared/${encodeURIComponent(token.value)}/meta`); meta.value = body.data; if (previewAllowed.value && !downloadExhausted.value) await loadPreview() } catch (err) { error.value = err.message } finally { loading.value = false } }
...
async function downloadFile() { if (downloadExhausted.value || downloadLoading.value) return; downloadLoading.value = true; downloadError.value = ''; try { const response = await fetch(downloadUrl.value); let body = null; if (!response.ok) { try { body = await response.json() } catch {} throw new Error(localizeError({ status: response.status, data: body?.data, backendMessage: body?.message })) } const blob = await response.blob(); const objectUrl = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = objectUrl; link.download = meta.value.fileName || 'download'; document.body.appendChild(link); link.click(); link.remove(); setTimeout(() => URL.revokeObjectURL(objectUrl), 0) } catch (err) { downloadError.value = err instanceof TypeError ? t('error.network') : (err.message || t('error.downloadFailed')) } finally { downloadLoading.value = false } }
