<template>
  <RouterLink v-if="link" to="/" class="brand-logo-link" :aria-label="brand.siteTitle">
    <div class="brand-logo" :class="{ compact }" role="img" :aria-hidden="true">
  ...
  <div v-else class="brand-logo" :class="{ compact }" :aria-label="brand.siteTitle" role="img">
    <img v-if="hasCustomAsset" :src="assetPath" :alt="brand.siteTitle" />
    <svg v-else viewBox="0 0 208 48" aria-hidden="true">
      <rect x="0" y="22" width="12" height="22" rx="2" fill="var(--brand-color)" />
      <rect x="17" y="5" width="12" height="39" rx="2" fill="#e76f51" />
...
<script setup>
// BrandLogo 根据页面变体选择自定义资源，并在缺失时使用内置 SVG；link=true 时点击跳转首页。
// BrandLogo selects a custom asset by page variant and falls back to the embedded SVG when absent; link=true wraps the logo in a router link to the home page.
import { computed } from 'vue'
import { RouterLink } from 'vue-router'
import { brand } from '../brand'

const props = defineProps({ variant: { type: String, default: 'main' }, compact: Boolean, link: Boolean })
