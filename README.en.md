...
FileBox is a self-hosted file transfer and management service that runs as a single Go binary on Windows or Linux. Stage 1 provides multi-user isolation, JWT login, single-file uploads, Range downloads, dual-hash verification, quotas, disk protection, audit logs, configurable branding, and a multilingual interface; Stage 2 (v0.2.0) adds resumable chunked uploads, instant upload, folder upload, share links, online preview, upload rate limiting, an optional public registration switch, and extended system statistics; the v012 batch further adds batch operations, external-user upload collections, per-user read-only windows, share management, and bidirectional SFTP sync; v016 completes 38 security and logic fixes from independent codex and DSH reviews; v017 delivers 10 new requirements (standalone collections page, per-tab admin descriptions, navigation order, log time-range filter, viewable sync credentials, execution history, path-picker enhancements, target-host display, real-time sync status, and a change-password modal).
...
- Batch operations (v012): file multi-select supports batch delete (`POST /api/files/batch-delete`) and batch zip download (`POST /api/files/batch-download`); the admin user list shows per-user folder/file counts and used bytes.
- Create-user security settings (v012): the create-user dialog can directly configure TOTP (one-time secret returned for handoff), TOTP re-enrollment, and the IP allowlist without a second edit; the brand settings theme color no longer occupies its own row.
- Public-proxy XFF trust toggle (v012): the `trustProxy` setting (default off) parses `X-Forwarded-For` only when enabled and the direct peer matches `--trusted-proxies`, preventing spoofed source IPs.
- External upload collections (v012): any logged-in user can create an upload-collection link (`/u/:token`) so external visitors can upload files without an account (reusing chunked/instant upload); limits include expiry, total upload count, and per-file size; files land under the creator's `uploads/<token>/` directory and count against their quota; revoke, optional remark, anonymous IP rate limiting, and audit are included.
- Per-user read-only window (v012): an admin can set a one-time window during which a user can only view/download; every write operation (upload/delete/rename/folders/share) returns `403 READ_ONLY`; admins are exempt and the frontend disables write entries with a notice.
- Share management (v012): the `/shares` page centrally manages my shares — list/detail (remaining time, downloads used/limit), extend expiry, increase the download limit (never lower), revoke a single link (soft delete; anonymous access returns 403 revoked), and copy links; each share's download log (time/IP/result/failure reason) is visible to its creator.
- Share failure reasons (v012): `share_not_found` / `share_expired` / `share_revoked` / `share_limit` / `share_denied` distinguish permission issues, exhausted downloads, expiry, and revocation.
...
- v020 user-feedback polish batch (11 items): all 12 native `window.confirm` calls (delete/batch-delete folders & files, bulk upload, revoke shares/groups/collections, delete sync tasks/systems, delete user, reset brand) replaced with queue-based FileBox confirm modals (zero `window.confirm` left); batch-download failures now surface a real cause (disk full / not writable / IO error) and the archive is named `filebox-batch-YYYYMMDD-HHMMSS.zip`; the transfers drawer supports batch pause/resume/terminate (upload terminate releases server quota via `DELETE /api/upload-tasks/{id}`, downloads resume over Range); the topbar Transfers entry shows icon+label; the duplicate refresh button on My collections was removed (plus a pagination `.value` residual fix); the share-management subtitle that wrongly showed "Copy link" is fixed (`shares.intro` split); the aggregate-share edit dialog matches the single-share dialog (info grid + link + hour-based extend/increase + member management); sync-log file details moved into a modal with per-file status badges; the sync detail dialog widened to 1280px; log failure reasons fully localized (trilingual `logReason.*` sets aligned, 38 keys each); the transfers icon now points up/down.
- v019 user-feedback fix batch: the log time-range panel gained Confirm/Clear buttons with a draft state (unconfirmed edits are ignored); pagination now shows on the Logs / My files / My shares / aggregate-share pages (template ref-unwrap bug); entering a folder no longer reports "invalid directory" (frontend path normalization + backend folder-list filtering that strips legacy `files/` prefixes and drops v010 backslash orphans and `.`/`..`); aggregate-share cards are trimmed to eye/copy/delete icons with editing merged into the eye dialog; the sync log dialog is widened to 1100px; folders support batch select/delete/rename.
- v018 enhancements: fixed the topbar "System settings" label (section→key mapping, admin→nav.system); aggregate shares support member add/remove and attribute editing (eye icon lists the file scope, edit dialog changes expiry/limit); collections put "Copy link" and "View received files" on the same row; in-flight sync tasks show live transfer progress (current file / files done / bytes / rate via polling `GET /api/sync/tasks/{id}/progress`); sync logs became a table (start/end/status/next-run/files/bytes/expandable detail); the log time filter is one "Time range" dropdown panel; My files / My shares / My collections / Logs pages gained per-page-size selectors (10/20/50/100) and page jumping; folders and files are merged into one table (folders first, click to enter).
- v017 enhancements: the file library drops its embedded collections block (collections live on the standalone "My collections" page); each admin tab has its own description text (trilingual); the topbar order is My files → My collections → My shares → Sync tasks → Logs → System settings ("Share management" renamed "My shares", "Admin" renamed "System settings"); the audit log page adds start/end time filters (`GET /api/logs?from=&to=`, RFC3339); sync remote-system passwords can be revealed via an eye toggle (`GET /api/sync/systems/{id}/secret`, single decrypted view + audit); sync task details show execution history (start/end times and in-flight state); the sync path picker supports current-directory filtering and manual full-path entry; task rows show the target host (SFTP host:port / FileBox URL host); sync runs write a running log row first and update it on completion (sync_logs migration adds finished_at); the topbar change-password entry opens a centered modal (forced change still uses the standalone page).
...
Under `--data`: `filebox.db`; with the default path, `data/files/<userID>[/<custom-dir>/]<stored-name>`, `data/tmp/<taskID>/<chunk-index>`, and `data/brand/<favicon|login-logo|main-logo>.<ext>`. SQLite stores metadata, ownership, quotas, audit logs, chunk records (`chunks`), share records (`shares` with soft-revoke marks), upload collections (`upload_collections`/`upload_collection_files`), and remote systems with sync tasks/logs (`remote_systems`/`sync_tasks`/`sync_logs`). Chunks are staged in the temporary directory and `complete` verifies them, then merges them as a stream into the user directory (optionally preserving a folder structure). The visible name is `name`, while the on-disk name is `stored_name`; conflicts use the smallest suffix such as `name (1).ext` and `name (2).ext`. Deletion marks `deleted`, subtracts quota, and removes physical content; storage paths held by soft-deleted records are automatically reused on re-upload. External collection uploads land under `data/files/<ownerID>/uploads/<token>/`; sync pull files are stored per task target directory and count against the owner's quota.
